---
title: "Events"
description: "An event is a financial business occurrence submitted to the Event Service for processing. The Event Service validates the event, evaluates templates, and produces double-entry transfers."
---

An **event** is a financial business occurrence submitted to the Event Service for processing. The Event Service validates the event, evaluates templates, and produces double-entry transfers.
...
4. **Transfer creation**: All transfers are submitted atomically. Either every transfer succeeds or none do.
5. **Publish**: The processed event envelope is published to the Event Store for audit.

## Idempotency

The `event_id` is the idempotency key. If processing fails, **retry with the same `event_id`**. Never generate a new `event_id` for a retry. The storage layer deduplicates transfers by ID, so resubmitting a previously successful event is safe.
