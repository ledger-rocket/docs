...
### Purpose-built financial storage

All transfers are written atomically to a purpose-built financial database that provides strict serializability, crash consistency, and high throughput for double-entry workloads.
...
|---------|----------------|
| **Ledger Service** | Reference data authority. Manages sites, entities, ledgers, accounts, account codes, currencies, and countries. |
| **Event Service** | Event processing engine. Accepts financial events, resolves accounts, evaluates templates, produces transfers, and commits them atomically. |
| **Balance Service** | Balance queries. Provides real-time and historical balance lookups with caching for immutable historical data. |
