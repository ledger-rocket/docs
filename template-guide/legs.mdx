...
"account_code_validations": [
    ...
    { "purpose": "suspense_account", "mask": "1003", "description": "..." }
]
```
...
"ledger_scope_descriptions": {
    "operational": "Operational ledger where Settled represents confirmed bank cash.",
    "fdic_tracking": "FDIC tracking ledger where Cash Balances positions are tracked."
}
```
...
A **bank statement ingestion** template records an inbound bank statement line on both the economic and allocation ledgers:
...
A **control orphan resolution** template moves funds through a suspense account in two steps:
