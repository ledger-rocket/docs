...
This validation from a **sweep reconciliation** template ensures the debit and credit sides of the reconciliation use different account codes -- preventing an event from accidentally debiting and crediting the same account.
...
This validation from a **suspense resolution** template prevents an operator from accidentally resolving a suspense balance back into the same suspense account.
...
### Sweep Reconciliation Bank Payment
...
Many production templates use empty validations, relying instead on `account_code_validations` and `metadata_schema` for input constraints.
