...
description: "How to trace financial events through the system using event chains and the Balance Service query endpoints."
---
...
```bash
curl -s "https://ledger.dev.ledgerrocket.com/v2/balance-service/api/v1/events/EVENT_UUID/chain?site_id=1" \
...
```bash
curl -s "https://ledger.dev.ledgerrocket.com/v2/balance-service/api/v1/events/EVENT_UUID/balance?site_id=1" \
  -H "x-api-key: lr-test-20250701" | jq .
```
...
```bash
curl -s "https://ledger.dev.ledgerrocket.com/v2/balance-service/api/v1/accounts/ACCOUNT_UUID/chain-balances?site_id=1" \
  -H "x-api-key: lr-test-20250701" | jq .
```
...
curl -s "https://ledger.dev.ledgerrocket.com/v2/event-service/api/v1/events/EVENT_UUID/transfers?site_id=1" \
  -H "x-api-key: lr-test-20250701" | jq .

# Enriched transfers (includes account names, entity details, ledger metadata)
curl -s "https://ledger.dev.ledgerrocket.com/v2/event-service/api/v1/events/EVENT_UUID/transfers/enriched?site_id=1" \
  -H "x-api-key: lr-test-20250701" | jq .
```
...
```bash
curl -s "https://ledger.dev.ledgerrocket.com/v2/event-service/api/v1/accounts/ACCOUNT_UUID/transfers?site_id=1" \
  -H "x-api-key: lr-test-20250701" | jq .
```
...
3. **Get the chain detail** to see all events in the problematic chain:
   ...
   ```bash
   curl -s "https://ledger.dev.ledgerrocket.com/v2/balance-service/api/v1/events/CHAIN_ROOT_UUID/chain?site_id=1" \
     -H "x-api-key: lr-test-20250701" | jq .
   ```
...
|---------|---------------|--------------|
| Clearing | Allocation file loaded but not reconciled | Missing consumer release step |
| Control | Bank statement ingested but not paired | Missing control flush step |
| Suspense | Clearing flushed but reconciliation incomplete | Missing bank payment reconciliation step |
| Unallocated | Detail lines loaded but consumers not matched | Missing consumer match step |
