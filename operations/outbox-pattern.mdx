...
| State | Meaning |
|-------|---------|
| `prepared` | Event accepted and validated. Transfers computed but not yet submitted to the financial database. |
| `committed` | Transfers atomically committed to the financial database. Balances are updated. |
...
4. Transfers are submitted atomically to the financial database
...
If publishing to the Event Store fails, the outbox retries with exponential backoff. The event remains in `committed` state during retries. Transfers are already committed to the financial database, so balances are correct regardless of publish status.
...
If events are stuck in `prepared` or `committed` after a restart, the service should automatically resume processing them. If they remain stuck after multiple restarts, check the financial database connectivity and service logs for errors during the prepare-to-commit or commit-to-publish transitions.
