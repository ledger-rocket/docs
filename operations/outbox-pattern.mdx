---
title: "Event Outbox Pattern"
description: "How events flow through the outbox state machine, and how to monitor and troubleshoot it."
---
...
| State | Meaning |
|-------|---------|
| `prepared` | Event accepted and validated. Transfers computed but not yet submitted to the ledger store. |
| `committed` | Transfers atomically committed to the ledger store. Balances are updated. |
...
4. Transfers are submitted atomically to the ledger store
...
If publishing to the Event Store fails, the outbox retries with exponential backoff. The event remains in `committed` state during retries. Transfers are already committed to the ledger store, so balances are correct regardless of publish status.
...
```bash
curl -s "https://ledger.dev.ledgerrocket.com/v2/event-service/api/v1/admin/outbox?site_id=1" \
...
```bash
# Only failed events
curl -s "https://ledger.dev.ledgerrocket.com/v2/event-service/api/v1/admin/outbox?site_id=1&state=failed" \
  -H "x-api-key: lr-test-20250701" | jq .
```
...
2. **Check the event details:**
   ...
   ```bash
   curl -s "https://ledger.dev.ledgerrocket.com/v2/event-service/api/v1/events/FAILED_EVENT_UUID?site_id=1" \
     -H "x-api-key: lr-test-20250701" | jq .
   ```
...
3. **Common failure causes:**
   ...
   | Transfer rejected by storage layer | Duplicate event_id or constraint violation | Check for duplicate submission; verify account/ledger setup |
...
If events are stuck in `prepared` or `committed` after a restart, the service should automatically resume processing them. If they remain stuck after multiple restarts, check the ledger store connectivity and service logs for errors during the prepare-to-commit or commit-to-publish transitions.
